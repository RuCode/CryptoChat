...
begin
  ...
  begin
    ...
    WriteDB('INSERT INTO ' + SQL_TBL_FRIENDS+'('+    SQL_COL_ID_FRIEND+', '+
      SQL_COL_ID_USER+', '+    SQL_COL_UUID+', '+    SQL_COL_NICK_NAME+', '+
      SQL_COL_EMAIL+') VALUES("' +    IntToStr (FCount) +  '", "' +
...
begin
  ...
  try
    ...
    WriteDB('INSERT INTO ' + SQL_TBL_MESSAGES+' VALUES("' +
      ...
      '", ' +      '"' + DateToStr(AMsg.Date) + '", ' +      '"' +
      TimeToStr(AMsg.Time) + '", ' +      '"' + IntToStr(AMsg.XID) +
      '", ' + '"' + szIsMy + '", ' + '"' + AMsg.OpenKey +'", ' +
...
begin
  ...
  SQLDataBase.ExecSQL ('CREATE TABLE IF NOT EXISTS ' + SQL_TBL_MESSAGES +
    ...
    SQL_COL_RESP_DATE +', '+SQL_COL_RESP_TIME+' TEXT, '+SQL_COL_XID+' INTEGER, '+
    ...
    try
      ...
      begin
        ...
        begin
          ...
          Result.Date     := StrToDate(SQLTable.FieldAsString (
            SQLTable.FieldIndex[SQL_COL_RESP_DATE]));
          Result.Time     := StrToTime(SQLTable.FieldAsString (
            SQLTable.FieldIndex[SQL_COL_RESP_TIME]));
