...
uses
  Classes, Dialogs, SysUtils, ExtCtrls, Forms, Controls, imapsend,
  ssl_openssl, synautil, synacode, synaicnv,
  mimemess, mimepart, synachar, smtpsend;

const
  ERROR_CAPTION = 'Ошибка';
  ERROR_INDEX_MESSAGE =
    'Индекс сообщения должен быть от 1 до числа равного количеству писем...';
  ERROR_RECIEVE_HEADER = 'Ошибка получения заголовка письма #%d';
  ERROR_RECIEVE_BODY = 'Ошибка получения тела письма #%d';

type
  TMailState = ({:Не загружено пиьсмо}msNone, {:Загружен заголовок}msMailHead,
    {:Загружено целиком}msMailAll);
  ...
  public
    ...
    function SendMail(ToAddr, Subject, Text: string; FileName: string = ''): boolean;
      overload;
    function SendMail(ToAddr, Subject, Text: string; AttachName: string;
  ...
  public
    ...
    {:Количество новых писем}
    property CountOfNewMails;
    {:Количество писем в директории}
    property CountOfMails;
    {:Результаты команды.}
    property FullResult;
    {:Результаты команды.}
...
begin
  if fIMAPClient.Password <> fSMTPClient.Password then
    raise Exception.Create(
      'Фатальная ошибка: Пароль пользователя должен быть одинаковым и для входящей и для исходящей почты...');
...
begin
  if fIMAPClient.UserName <> fSMTPClient.UserName then
    raise Exception.Create(
      'Фатальная ошибка: Имя пользователя должно быть одинаковым и для входящей и для исходящей почты...');
...
begin
  ...
  for i := 0 to ListFolders.Count - 1 do
    ListFolders[i] := CharsetConversion(ListFolders[i], TMimeChar.UTF_7mod,
      TMimeChar.UTF_8);
end;
...
begin
  ...
  begin
    MessageDlg(ERROR_CAPTION, Format(ERROR_RECIEVE_HEADER, [Index]),
      mtError, [mbOK], '');
    Exit;
  end;
...
begin
  ...
  try
    ...
    Result := True;
    Result := Result and fSMTPClient.MailFrom(fSMTPClient.UserName,
      Length(fSMTPClient.UserName));
...
function TCustomMail.SendMail(ToAddr, Subject, Text: string;
  AttachName: string; Stream: TStream): boolean;
  // Отправка письма
var
  MailMessage: TMimeMess;
  MIMEPart: TMimePart;
...
begin
  ...
  try
    ...
    // Вложение
    MIMEPart := MailMessage.AddPartMultipart('attachment', nil);
    MailMessage.AddPartBinary(Stream, AttachName, MIMEPart);
    // Кодируем и отправляем
    MailMessage.EncodeMessage;
    Result := True;
    Result := Result and fSMTPClient.MailFrom(fSMTPClient.UserName,
...
begin
  if Assigned(fSMTPClient) then
    fSMTPClient.NoOp;
  if Assigned(fIMAPClient) then
    fIMAPClient.NoOp;
end;
