...
type
  ...
  private
    ...
    procedure ContactDblClick(Sender: TObject);
  ...
  public
    ...
    //* ======================================================================== *//
    procedure Recv(AFriendID: integer; AText: string; ADate: TDateTime; AFiles: TFiles); overload;
    procedure Send(AFriendID: integer; AText: string; ADate: TDateTime; AFiles: TFiles); overload;
    procedure Recv(AText: string; ADate: TDateTime; AFiles: TFiles); overload;
    procedure Send(AText: string; ADate: TDateTime; AFiles: TFiles); overload;
    //* ======================================================================== *//
...
function TCustomCryptChat.Add(AFriendID: integer; AText: string; ADate: TDate;
  Avatar: string; AFiles: TFiles; State: TMsgState): boolean;
var
...
begin
  ...
  begin
    ...
    if not GetFriendImage(AFriendID, '/tmp/friend.png') then
         CopyFile(PChar(ExtractFilePath(ParamStr(0))+'Data/def_fava.png'),  PChar('/tmp/friend.png'), TRUE);
...
begin
  fListBox.Clear;
  fUpdateContactList;
  { TODO : Проверить есть ли входящие }
  { TODO : Провести чтение входящих  }
  { TODO : Отобразить изменения }
end;
...
begin
  ...
  begin
    ...
    if not GetUserImage(GetUserInfo.ID_USER, Path) then
      CopyFile(PChar(ExtractFilePath(ParamStr(0)) + 'Data' + slash + 'def_ava.png'),
        PChar(Path), True);
  end;
end;
