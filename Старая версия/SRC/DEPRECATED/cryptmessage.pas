...
{$ENDIF UNIX}
{$IFDEF WINDOWS}
TMessages = class(TCustomPanel) // Какого то в грёбаной винде не может менять цвет для TCustomControl =(
{$ENDIF WINDOWS}
private
  fImg    :TImage;
  fName   :TLabel;
...
begin
  ...
  {$IFDEF WINDOWS}
  (self as TMessages).BorderStyle:= bsNone;
  (self as TMessages).BevelInner:=  bvNone;
  (self as TMessages).BevelOuter:=  bvNone;
  {$ENDIF WINDOWS}
  // Avatar
  ...
  fText.WordWrap := True;
  fText.AutoSize := True;
  //fText.Constraints.MaxWidth:= (self as TMessages).Width;
  // Date of Message
